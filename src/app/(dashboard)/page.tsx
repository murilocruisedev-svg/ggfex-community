...
import { ArrowRight, Sparkles, Music, Loader2, Search } from 'lucide-react'
...
function HomeContent() {
    ...
    return (
        <div className="space-y-6 md:space-y-12 pb-12 font-sans bg-[#050505]">
            ...
            {/* Banner - esconde quando pesquisando */}
            {!isSearching && (
            ...
            {/* Seção de Conteúdo */}
            <section>
                {/* Header: Resultados da busca OU Galeria Completa */}
                {isSearching ? (
                    <div className="mb-8 space-y-2">
                        <h2 className="text-2xl md:text-3xl text-white font-bold tracking-tight flex items-center gap-3">
                            <Search className="w-6 h-6 text-[#1F51FF]" />
                            Resultados para &quot;{searchQuery}&quot;
                        </h2>
                        <p className="text-sm text-gray-500 ml-9">
                            {filteredSounds.length} {filteredSounds.length === 1 ? 'resultado encontrado' : 'resultados encontrados'}
                        </p>
                    </div>
                ) : (
